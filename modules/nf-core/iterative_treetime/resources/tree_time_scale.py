...
from fig_utils import year_decimal_to_date_tick_labels
import io
import os
...
def _parse_dates_robust(fdates, name_col=None, date_col='date'):
    ...
    """
    from date_utilities import date_to_decimal
...
def plot_temporal_tree(time_tree):
    """
    Plot temporal tree using baltic with a cosmetic calendar time axis.

    Styled after the baltic "cosmetic time axis" gallery example
    (https://phylo-baltic.github.io/baltic-gallery/cosmetic-time-axis/):
    branch lengths are in years, the tree is drawn against absolute calendar
    time, only the bottom spine is shown, and alternating shaded vertical bands
    mark successive tick intervals.
    ...
    # Most recent tip date calibrates baltic's absolute time axis.
    tip_dates = [t.numdate for t in tips
                 if hasattr(t, 'numdate') and t.numdate is not None]
    most_recent = max(tip_dates) if tip_dates else None

    # Set root branch length to 0 so the root sits at the tree's start.
    time_tree.tree.root.branch_length = 0.0

    # Serialise the temporal tree to newick and load into baltic.
    handle = io.StringIO()
    Phylo.write(time_tree.tree, handle, format='newick',
                format_branch_length='%1.8f')
    bt_tree = bt.loadNewick(io.StringIO(handle.getvalue()), absoluteTime=False)
    bt_tree.traverse_tree()
    bt_tree.sortBranches()
    if most_recent is not None:
        bt_tree.setAbsoluteTime(most_recent)

    fig_height = max(8, nleafs * 0.2)
    fig, ax = plt.subplots(1, 1, figsize=(15, 10), facecolor='w')

    x_attr = lambda k: k.absoluteTime
    bt_tree.plotTree(ax, x_attr=x_attr, colour='k', width=1)
    bt_tree.plotPoints(ax, x_attr=x_attr, size=20, colour='w', zorder=100)

    # Tip labels only when the tree is small enough to be legible.
    if nleafs < 30 and most_recent is not None:
        bt_tree.addText(
            ax,
            x_attr=lambda k: k.absoluteTime + bt_tree.treeHeight * 0.01,
            text=lambda k: k.name,
            verticalalignment='center', fontsize=8,
        )

    if most_recent is not None:
        abs_times = [k.absoluteTime for k in bt_tree.Objects
                     if getattr(k, 'absoluteTime', None) is not None]
        x_min, x_max = min(abs_times), max(abs_times)

        # Cosmetic calendar time axis with a monthly cadence, styled after the
        # baltic "cosmetic time axis" gallery example. Ticks are placed at the
        # first of every month; labels show the 3-letter month name, with the
        # year on a second row at each January.
        start_year, start_month = int(x_min // 1), int((x_min % 1) * 12) + 1
        end_year = int(x_max // 1)
        end_month = int((x_max % 1) * 12) + 1

        calendar_timeline = []
        year, month = start_year, start_month
        while (year, month) <= (end_year, end_month):
            calendar_timeline.append('%04d-%02d-01' % (year, month))
            month += 1
            if month > 12:
                month, year = 1, year + 1

        # Tick position at the middle of each month (~15 days after month start).
        tick_positions = [bt.decimalDate(t) + 15 / 365 for t in calendar_timeline]

        # 3-letter month name, plus the year on a second row for each January.
        # Alternate months are blanked to avoid crowding over a wide span
        # (January/year labels are always kept).
        tick_labels = []
        for i, d in enumerate(calendar_timeline):
            if d.split('-')[1] == '01':  # January: always label with year row
                tick_labels.append(bt.convertDate(d, '%Y-%m-%d', '%b\n%Y'))
            elif i % 2 == 0:  # thin: label every other month
                tick_labels.append(bt.convertDate(d, '%Y-%m-%d', '%b'))
            else:
                tick_labels.append('')

        # Alternating shaded vertical bands spanning every other month.
        for i, d in enumerate(calendar_timeline):
            if i % 2 == 0:
                ax.axvspan(bt.decimalDate(d), bt.decimalDate(d) + 30 / 365,
                           facecolor='k', edgecolor='none', alpha=0.08, zorder=0)

        ax.set_xticks(tick_positions)
        ax.set_xticklabels(tick_labels)
        ax.set_xlim(x_min - bt_tree.treeHeight * 0.02,
                    x_max + bt_tree.treeHeight * 0.02)

    # Minimal styling: only the bottom spine, no y ticks/labels.
